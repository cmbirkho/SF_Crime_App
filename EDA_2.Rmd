...
```{r}
set.seed(54321)
indexes <- createDataPartition(df$incident_time_of_day,
                               times = 1,
                               p = 0.7,
                               list = FALSE)
...
caret.cv <- train(incident_time_of_day ~ incident_day_of_week + incident_category +
                    police_district,
                  data = training,
                  method = "rf",
...
confusionMatrix(prd, testing$incident_category)
```
