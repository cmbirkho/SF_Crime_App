...
#-------------------------------------------------------------------------------
# Clean up dates, make new datetime columns, apply date filter
sfCrime <- sfCrime[, 
...
# Exclude data where lat and long is not available
sfCrime <- sfCrime[!is.na(latitude) & !is.na(longitude), ]

#-------------------------------------------------------------------------------
# Create an incident_value column
# extract the whole string before "("
sfCrime$incident_value <- sub("*\\(.*", "", sfCrime$incident_description) 
